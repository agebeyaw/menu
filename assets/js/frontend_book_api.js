    ...
    exports.getAvailableHours = function (selDate) {
        ...
        $.post(postUrl, postData, function (response) {
            ...
            if (response.length > 0) {
                var currColumn = 1;
                $('#available-hours').html('<div style="width:78px; float:left;"></div>');
                ...
                $.each(response, function (index, availableHour) {
                    if ((currColumn * 10) < (index + 1)) {
                        currColumn++;
                        $('#available-hours').append('<div style="width:78px; margin-left:4px; float:left;"></div>');
                    }

                    $('#available-hours div:eq(' + (currColumn - 1) + ')').append(
                        '<span class="available-hour btn">' + Date.parse(availableHour).toString(timeFormat) + '</span><br/>');
                });
