...
(function (exports) {
    ...
    function _bindEventHandlers() {
        ...
        $('#manage-appointment #save-appointment').click(function () {
            ...
            var appointment = {
                ...
                number_of_people: $dialog.find('#number_of_people').val(),
        ...
        $('#select-service').change(function () {
            ...
            _updateNumberOfPeopleSelection()

        });


        /**
         * Event: Selected Provider "Change"
         *
         * When the user selects a provider, we need to update the number of people selection box according to provider configuration.
         */
    ...
    function _updateNumberOfPeopleSelection() {
        var pid = $('#select-provider').val();
        var number_of_people = $('#number_of_people').val();
        var maxLimit = 1;

        // Update the number_of_people select box.
        $.each(GlobalVariables.availableProviders, function (indexProvider, provider) {
            // Fill the number of people selection box until the maximum allowed by the provider.
            if (provider.id == pid) {
                maxLimit = provider.settings.max_people_in_appointment || 1;
                return true;
            }
        });

        $('#number_of_people').empty();
        for (let people = 1; people <= maxLimit; people++) {
            var optionHtml = '<option value="' + people + '">' + people + '</option>';
            $('#number_of_people').append(optionHtml);
        }
        
        $('#number_of_people').val(number_of_people);
    }
