...
(function () {
    ...
    ProvidersHelper.prototype.bindEventHandlers = function () {
        ...
        const menuJsonEditorContainer = document.getElementById("provider_menu");
        const jsonEditorOptions = {};
        this.menuEditor = new JSONEditor(menuJsonEditorContainer, jsonEditorOptions);
        ...
        $('#providers').on('click', '#save-provider', function () {
            var provider = {
                ...
                settings: {
                    ...
                    address_show: $('#provider-address_show').prop('checked'),
                    enable_social_login: $('#provider-enable_social_login').prop('checked'),
                    trending: $('#provider-trending').prop('checked'),
                    always_require_approval: $('#provider-always_require_approval').prop('checked'),
                    provider_logo_url: $('#provider-provider_logo_url').val(),
                    provider_moto: $('#provider_moto').val(),
                    max_people_in_appointment: $('#provider-max_people_in_appointment').val(),
                    whatsapp_click_to_chat_link: $('#provider-whatsapp_click_to_chat_link').val(),
                    phone_country_code: $('#provider_phone_country_code').val(),
                    latitude: $('#provider_latitude').val(),
                    longitude: $('#provider_longitude').val(),
                    timezone: $('#provider_timezone').val(),
                    locale: $('#provider_locale').val(),
                    hour_format: $('#provider_hour_format').val(),
                    main_image_url: $('#provider_main_image_url').val(),
                    gallery_urls: $('#provider_gallery_urls').val(),
                    cuisines: $('#provider_cuisines').val(),
                    amenities: $('#provider_amenities').val(),
                    rating: $('#provider_rating').val(),
                    menu: JSON.stringify(this.menuEditor.get()),
                    price_level: $('#provider_price_level').val(),
                }
            };
    ...
    ProvidersHelper.prototype.display = function (provider) {
        if (provider.settings.menu == null) {
            const initialJson = [
                {
                    "itemId": 1,
                    "section": "MAIN COURSES",
                    "itemName": "Example menu item name 1",
                    "itemCost": 2,
                    "itemDescription": "description of the example menu item name 1",
                    "veg": true,
                    "orderCount": 0
                }
            ];
            this.menuEditor.set(initialJson);
        } else {
            this.menuEditor.set(JSON.parse(provider.settings.menu));
        }
        ...
        $('#provider-provider_logo_url').val(provider.settings.provider_logo_url);
        $('#provider_moto').val(provider.settings.provider_moto);
        $('#provider-whatsapp_click_to_chat_link').val(provider.settings.whatsapp_click_to_chat_link);
        $('#provider-max_people_in_appointment').val(provider.settings.max_people_in_appointment);
        $('#provider_phone_country_code').val(provider.settings.phone_country_code);
        $('#provider_latitude').val(provider.settings.latitude);
        $('#provider_longitude').val(provider.settings.longitude);
        $('#provider_timezone').val(provider.settings.timezone);
        $('#provider_locale').val(provider.settings.locale);
        $('#provider_hour_format').val(provider.settings.hour_format);
        $('#provider_main_image_url').val(provider.settings.main_image_url);
        $('#provider_gallery_urls').val(provider.settings.gallery_urls);
        $('#provider_cuisines').val(provider.settings.cuisines);
        $('#provider_amenities').val(provider.settings.amenities);
        $('#provider_rating').val(provider.settings.rating);
        $('#provider_price_level').val(provider.settings.price_level);

        if (provider.settings.enable_social_login == 1) {
            $('#provider-enable_social_login').prop('checked', true);
        } else {
            $('#provider-enable_social_login').prop('checked', false);
        }

        if (provider.settings.trending == 1) {
            $('#provider-trending').prop('checked', true);
        } else {
            $('#provider-trending').prop('checked', false);
        }

        if (provider.settings.always_require_approval == 1) {
            $('#provider-always_require_approval').prop('checked', true);
        } else {
            $('#provider-always_require_approval').prop('checked', false);
        }

        if (provider.settings.address_show == 1) {
            $('#provider-address_show').prop('checked', true);
        } else {
            $('#provider-address_show').prop('checked', false);
        }
    ...
    ProvidersHelper.prototype.getFilterHtml = function (provider) {
        var name = provider.first_name + ' ' + provider.last_name + ' (' + provider.id + ')',
            info = provider.email;
